...
# ---------- Thresholds ----------
EYE_CLOSED_THRESHOLD = 0.40   # <-- updated (very sensitive)
MOUTH_OPEN_THRESHOLD = 0.16

# ---------- Frame thresholds ----------
CLOSED_FRAMES_THRESHOLD = 1
YAWN_FRAMES_THRESHOLD = 18
...
while True:
    ...
    if result.face_landmarks:
        ...
        # Debug print
        print(
            f"Left EAR: {left_ear:.3f} | Right EAR: {right_ear:.3f} | "
            f"Avg EAR: {avg_ear:.3f} | Mouth: {mouth_ratio:.3f} | "
            f"ClosedFrames: {closed_eye_frames}",
            end="\r"
        )
    ...
    # ---------- Status ----------
    cv2.putText(frame, f"Status: {status_text}", (20, 180),
